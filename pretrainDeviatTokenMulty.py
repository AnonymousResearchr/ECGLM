...
from tokenizer import DeviatTokenizer  # Modified to use DeviatTokenizer
...
class ECGDataset(Dataset):
    """
    Custom dataset class for handling ECG data.
    """
...
class ResMamba(nn.Module):
    """
    Residual Mamba model with a Mamba2 block and SELU activation.
    """
    ...
    def forward(self, x):
        """
        Forward pass through the ResMamba model with reshaping and padding.
        """
        batch_size = x.size(0)
        # Reshape input
...
def load_file(file_path):
    """
    Load a single HDF5 file and transfer to GPU if it has the expected shape.
    
    Args:
    - file_path: Path to the HDF5 file.
    
    Returns:
    - Data tensor or error message string.
    """
    try:
...
def process_all_data(split_files, device, log):
    ...
    Returns:
    - List of processed data tensors.
    """
    all_data = []
...
def split_dataset(all_data):
    ...
    Args:
    - all_data: List of all data tensors.
