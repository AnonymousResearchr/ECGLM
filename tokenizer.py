...
class NullTokenizer:
    ...
    def tokenize(self, data):
        """
        Tokenizes the input ECG data by padding and reshaping.
        """
        b, l, c = data.shape
...
class ECGMetaTokenizer(nn.Module):
    """
    A tokenizer class that extracts meta features from ECG data and pads it to the target length.
    """
    def __init__(self, chunk_size=250, num_chunks=256, target_length=270):
        ...
        self.num_chunks = num_chunks
        self.target_length = target_length
        self.mask = nn.Parameter(torch.ones((self.target_length - self.chunk_size), 1))  # Mask parameter
    ...
    def _pad_data_block(self, data_block):
        ...
        meta_features = self.metaFeature(data_block) * self.mask  # Apply mask

        # Compute the mean and standard deviation of meta_features
        mean = meta_features.mean(dim=0, keepdim=True)
        std = meta_features.std(dim=0, keepdim=True)

        # Normalize meta_features
...
class ECGSpaceTokenizer(nn.Module):
    """
    A tokenizer class that generates sparse spatial features from ECG data and pads it to the target length.
    """
    ...
    def SparseSpatialFeature(self, spatial_matrix):
        ...
        for i in range(240):
            ...
            sparse_matrix = self.sparse_feature_matrix[i].unsqueeze(0).expand(b, -1, -1)  # Expand batch dimension
        ...
        # Normalize the spatial features along the first dimension
        spatial_features_mean = multi_lead_splines.mean(dim=1, keepdim=True)
        spatial_features_std = multi_lead_splines.std(dim=1, keepdim=True)
        multi_lead_splines = (multi_lead_splines - spatial_features_mean) / (spatial_features_std + 1e-8)
    ...
    def _pad_data_block(self, data_block, spatial_features):
        ...
        spatial_features = spatial_features[:, :pad_size, :].to(data_block.device)  # Ensure on GPU
        return torch.cat([data_block, spatial_features], dim=1)
    ...
    def tokenize(self, data, data_time_padded):
        ...
        spatial_features = self.SparseSpatialFeature(spatial_matrix)
        padded_data = self._pad_data_block(data_time_padded, spatial_features)
        return padded_data
...
class CrossTokenizer(nn.Module):
    """
    A tokenizer class that generates cross-lead spatial features from ECG data and normalizes it.
    """
    ...
    def SparseSpatialFeature(self, ecg_data):
        """
        Generates sparse spatial features for each lead of the ECG data.
        """
    ...
    def tokenize(self, data):
        """
        Tokenizes the data by generating cross-lead spatial features and normalizing them.
        """
        b, l, c = data.shape
...
class DeviatTokenizer(nn.Module):
    """
    A tokenizer class that uses FFT-based hashing and the Mamba2 block to process ECG data.
    """
    def __init__(self, d_model=256, d_state=64, d_conv=4, dropout_rate=0.1, codebook_shape=(500, 5000, 12)):
        ...
        self.hash_table = nn.Parameter(torch.rand(codebook_shape[0], 12 * 2))  # Hash value for each codebook
        self.mamba2_block = Mamba2(d_model=d_model, d_state=d_state, d_conv=d_conv, dropout_rate=dropout_rate)

    def fft_hash(self, data):
        """
        Computes FFT-based hash indices for the input data.
        """
        fft_features = torch.fft.rfft(data, dim=1)  # Shape: (batch_size, frequency_bins, 12)
        magnitude = torch.abs(fft_features)  # Get magnitude
        phase = torch.angle(fft_features)  # Get phase
        ...
        # Simple hash calculation: compare the mean of each codebook in the hash table with the mean of the current data, and find the most similar one
    ...
    def forward(self, data, codebook):
        """
        Forward pass through the tokenizer, including FFT-based hashing and Mamba2 block processing.
        """
